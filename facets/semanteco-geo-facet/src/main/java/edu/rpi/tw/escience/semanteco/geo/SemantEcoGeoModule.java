...
public class SemantEcoGeoModule implements Module, ProvidesDomain {
   ...
   * entry.setLabel("bird1"); // entry.setAltLabel("birdicus uno"); //
   * items.add(entry); // entry = new HierarchyEntry(); //
   * entry.setUri(URI.create("http://example.com/bird2")); //
   * entry.setLabel("bird2"); // entry.setAltLabel("birdicus dos"); //
   * items.add(entry); return queryedarrinTaxonomyHMRoots(request); } else if
   * (action == HierarchyVerb.CHILDREN) { // if ( //
   * request.getParam("species").equals("http://example.com/bird1") ) // { //
   * HierarchyEntry entry = new HierarchyEntry(); //
   * entry.setUri(URI.create("http://example.com/bird3")); //
   * entry.setLabel("bird3"); // entry.setAltLabel("birdicus tres"); //
   * items.add(entry); // entry = new HierarchyEntry(); //
   * entry.setUri(URI.create("http://example.com/bird4")); //
   * entry.setLabel("bird4"); // entry.setAltLabel("birdicus quatro"); //
   * items.add(entry); // } return querydarrinTaxonomyHMChildren(request,
   * (String) request.getParam("darrinspecies")); } else if (action ==
   * HierarchyVerb.SEARCH) { return searchDarrinSpecies(request, (String)
   * request.getParam("string")); } else if (action ==
   * HierarchyVerb.PATH_TO_NODE) { return darrinSpeciesPathToNode(request,
   * (String) request.getParam("uri")); } return items; }
   */
  /*
   * protected Collection<HierarchyEntry> queryedarrinTaxonomyHMRoots(final
   * Request request) { Collection<HierarchyEntry> entries = new
   * ArrayList<HierarchyEntry>(); // Variables final Query query =
   * config.getQueryFactory().newQuery(Type.SELECT);
   * 
   * final Variable id = query.getVariable(VAR_NS + "child"); final Variable
   * label = query.getVariable(VAR_NS + "label"); final Variable parent =
   ...
   * subClassOf, parent); graph.addPattern(id, subClassOf, darrinTaxonomy);
   ...
   * Variables final Variable id = query.getVariable(VAR_NS + "child"); final
   * Variable label = query.getVariable(VAR_NS + "label"); final Variable parent
   * = query.getVariable(VAR_NS + "parent"); // URIs final QueryResource
   * subClassOf = query.getResource(RDFS_NS + "subClassOf"); final QueryResource
  ...
  public void visit(SemantEcoUI ui, Request request) {
    ...
    Resource res2 = null;
    /*
     * res = config.getResource("darrinSpeciesHierarchy.js"); res2 =
     * config.getResource("darrinSpeciesHierarchy.jsp");
     * 
     * ui.addScript(res); ui.addFacet(res2);
     */
