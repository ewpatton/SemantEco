...
/**
 * Provides data on bird and fish species and a hierarchical facet to browse
 * through the taxonomical hierarchy to specialize queries to a collection of
 * species.
 * @author apseyed
 * @author ewpatton
 *
...
public class SpeciesDataProviderModule implements Module, ProvidesDomain {
	...
	public static final String FISH_E1_NS = "http://was.tw.rpi.edu/source/fish-data/dataset/sbc-data/vocab/enhancement/1/";
	...
	/**
	 * 
	 * this executes the query on the remote endpoint and provides the results
	...
	public void visit(final Model model, final Request request,
		...
		if (domainUri.equals(FISH_NS)) {
			...
			final QueryResource fishSite = query.getResource(FISH_NS
					+ "FishSite");
			...
			// not correct
			// http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/vocab/enhancement/1/locality/locality
			// what is in the rdf:
			// @prefix e1:
			// <http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/vocab/enhancement/1/>
			// .
			// final QueryResource siteUri = query.getResource(Site); //update
			// locality property namespace
			// species site and measurement (count)
			// just the uri, lat and long.
			final QueryResource countyCoded = query
					.getResource(FISH_E1_NS + "countycoded");
			final QueryResource stateAbbrev = query
					.getResource(FISH_E1_NS + "statecoded");
			final QueryResource site = query
					.getResource(FISH_E1_NS + "site");
			...
			final GraphComponentCollection graph = query
					.getNamedGraph("http://was.tw.rpi.edu/fish-data");
			final GraphComponentCollection graph2 = query
					.getNamedGraph("http://was.tw.rpi.edu/fish-sites");
			...
			// this executes the query on the remote endpoint and provides the
			// results to the model passed in
			config.getQueryExecutor(request).accept("text/turtle")
					.execute(query, model);
		} else if (domainUri.equals(BIRD_NS)) {
			String countyCode = (String) request.getParam("county");
			String stateAbbr = (String) request.getParam("state");
			// String site = (String) request.getParam("uri");
			assert (countyCode != null);
			...
			final QueryResource wgsLat = query.getResource(WGS_NS + LAT);
			final QueryResource wgsLong = query.getResource(WGS_NS + LONG);
			final QueryResource birdSite = query
					.getResource(BIRD_NS + "BirdSite");
			final Variable lat = query.getVariable(QUERY_NS + LAT);
			final Variable lng = query.getVariable(QUERY_NS + LONG);
			final Variable label = query.getVariable(QUERY_NS + "label");
			final Variable measurement = query.getVariable(QUERY_NS
			...
			final QueryResource locality = query
					.getResource(EBIRD_DATA_NS + "locality");
			// final QueryResource siteUri = query.getResource(Site); //update
			// locality property namespace
			// species site and measurement (count)
			// just the uri, lat and long.
			// the namespace for countyCode and StateCoded in the fish data
			final QueryResource countyCoded = query.getResource(e1_NS
					+ "countyCoded");
			final QueryResource stateAbbrev = query.getResource(e1_NS
					+ "stateCoded");
			...
			// query.getNamedGraph("http://was.tw.rpi.edu/ebird-data2");
			final GraphComponentCollection graph = query
					.getNamedGraph("http://was.tw.rpi.edu/ebird-data-ca");
			final QueryResource hasScientificName = query.getResource(e2_NS
					+ "scientific_name");
			final Variable scientificName = query.getVariable(VAR_NS
					+ "scientific_name");
			...
			JSONArray speciesList = (JSONArray)request.getParam("species");
			if (speciesList != null && speciesList.length() > 1) {
				request.getLogger().error(
						"species length: "
								+ ((JSONArray) request.getParam("species"))
										.length());
				...
				JSONArray speciesParams = (JSONArray) request
						.getParam("species");
				...
				for (int i = 0; i < speciesParams.length(); i++) {
					...
					// speciesParams.getJSONObject(i);
					String speciesInArray = null;
					speciesInArray = speciesParams.optString(i);
					// request.getLogger().error("JSON Object: " +
					// speciesInArray.toString());
					...
					// superClassOfSpecies is a (kingdom, phylum, or x
					// (non-species)) and
					...
					// if that is the case, then add 'addedSpecies, hasLabel,
					// scientificName' to the union

					// two graphs for this:
					// http://was.tw.rpi.edu/ebird-taxonomy
					// http://was.tw.rpi.edu/ebird-data
					// namespace of scientificName:
					// http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/vocab/enhancement/1/

					// here is where I can check results of subClassOf hierarchy
					// query, and then union it
					String resultStr = null;
					try {
						resultStr = queryIfTaxonomicCategory(request,
								speciesInArray);
					} catch (JSONException e) {
						request.getLogger().warn("Unexpected exception", e);
					}
					// if there are any bindings union up
					if (resultStr != "FAILURE") {
						request.getLogger().error(
								"subclassOf results: " + resultStr);
						...
						// data.length is > 0 then there were positive results,
						// so now we can ask for subclasses of selection
						request.getLogger().error(
								"data.length : " + data.length());

						if (data.length() > 0) {
							// just use the pattern species subClassOf
							// speciesSelection
							final QueryResource addedSpecies = query
									.getResource(speciesInArray);
							final QueryResource subClassOf = query
									.getResource(RDFS_NS + "subClassOf");
							coll = union.getUnionComponent(i);
							// final NamedGraphComponent graph3 =
							// query.getNamedGraph("http://was.tw.rpi.edu/ebird-taxonomy");
							...
							// ebird data graph (already handled in the first
							// part of the query)
							// graph3.addPattern(addedSpecies, hasLabel,
							// scientificName);
							/*
							 * for(int n = 0; n < data.length(); n++) {
							 * JSONObject objectInArray = data.getJSONObject(n);
							 * String speciesForQuery =
							 * objectInArray.get("species").toString();
							 * request.getLogger().error("species for query: " +
							 * speciesForQuery); final QueryResource
							 * addedSpecies =
							 * query.getResource(speciesForQuery); coll =
							 * union.getUnionComponent(n);
							 * coll.addPattern(addedSpecies, hasLabel,
							 * scientificName); }
							 */
						} else {
							request.getLogger()
									.error("(no results for queryIfTaxonomicCategory)");
							final QueryResource addedSpecies = query
									.getResource(speciesInArray);
							coll = union.getUnionComponent(i);
							coll.addPattern(addedSpecies, hasLabel,
						...
						request.getLogger().error(
								"(failure to queryIfTaxonomicCategory)");
					}
				}
			} else if (speciesList != null && speciesList.length() == 1) {
				request.getLogger()
						.error("(got to else if where species == 1)");

				JSONArray speciesParams = (JSONArray) request
						.getParam("species");
				String speciesInArray = speciesParams.optString(0);
				String resultStr = null;
				try {
					resultStr = queryIfTaxonomicCategory(request,
							speciesInArray);
				...
				if (resultStr != "FAILURE") {
					...
					JSONArray data = (JSONArray) results.optJSONArray("data");
					// data.length is > 0 then there were positive results, so
					// now we can ask for subclasses of selection
					...
					if (data.length() > 0) {
						...
						request.getLogger().error(
								"addedSpecies: " + addedSpecies.toString());
						request.getLogger().error(
						...
						request.getLogger().error(
								"(no results for queryIfTaxonomicCategory)");
				...
				final NamedGraphComponent graph2 = query
						.getNamedGraph("http://was.tw.rpi.edu/ebird-taxonomy");
				graph2.addPattern(species, hasLabel, scientificName);
			}// for this case the species is left as a variable and is not
				// constrained, "all bird counts" for the county
	...
	public void visit(final OntModel model, final Request request,
		...
		model.read(BIRD_NS);
		model.read(FISH_NS);
		// create a bird.owl that imports pollution.owl and asserts a subclass
		// of Measurement site
		// that is a BirdSite used as class, the enhancement in the rdf eBird
		// data.
		// in the enhancement, need to promote location to a uri, and make it a
		// subclass, BirdSite.
	...
	public void visit(final Query query, final Request request) {
		...
		final QueryResource rdfType = query.getResource(RDF_NS + "type");
		final QueryResource polMeasurementSite = query.getResource(POL_NS
				+ "MeasurementSite");
		List<GraphComponentCollection> graphs = query
				.findGraphComponentsWithPattern(site, rdfType,
						polMeasurementSite);
		if (graphs != null && graphs.size() > 0) {
			...
			if (bird) {
				final Variable isBird = query
						.createVariableExpression("EXISTS { ?"
								+ SITE_VAR
								+ " a <http://escience.rpi.edu/ontology/semanteco/2/0/bird.owl#BirdSite> } as ?"
								+ ISBIRD_VAR);
				vars.add(isBird);
			}
			if (fish) {
				final Variable isFish = query
						.createVariableExpression("EXISTS { ?"
								+ SITE_VAR
								+ " a <http://escience.rpi.edu/ontology/semanteco/2/0/fish.owl#FishSite> } as ?"
								+ ISFISH_VAR);
				vars.add(isFish);
			}
	...
	/**
	 * A test method that returns a string indicating the type of object the
	 * species key contains
	 * 
	 * @param request
	 * @return
	...
	public String queryParams(Request request) throws JSONException {
		if (request.getParam("species") != null) {// &&
													// request.getParam("species").length()
													// > 0) {
			// throw new
			// IllegalArgumentException("The source parameter must be supplied");
			...
			for (int i = 0; i < speciesParams.length(); i++) {
				...
				request.getLogger().error(
						"JSON Object: " + objectInArray.toString());
				// here is where you do the union
			}
	...
	/*
	 * This method will let the calling UI client know if the class is a leaf or
	 * a non-leaf node in the class hierarchy. checks the bbq state value for
	 * the key: "queryIfTaxonomicCategoryForJsTree",
	 * 
	 * so for testing: $.bbq.pushState({"queryIfTaxonomicCategoryForJstree":
	 * "http://ebird#Struthionidae"})
	 * 
	 * returns: {"data":[{"species":"http://ebird#Struthio_camelus"},{"species":
	 * "http://ebird#Struthio_camelus_molybdophanes"}],"success":true}
	 * 
	 * but if I try with a leaf node:
	 * $.bbq.pushState({"queryIfTaxonomicCategoryForJstree"
	 * :"http://ebird#Nothocercus_bonapartei_frantzii"})
	 * 
	 * you get: {"data":[],"success":true}
	 */

	/**
	 * This method lets the calling UI client know if the class is a leaf or a
	 * non-leaf node in the class hierarchy. checks the bbq state value of
	 * "queryIfTaxonomicCategoryForJsTree".
	 * 
	 * @param request
	 * @return
	 * @throws JSONException
	...
	public String queryIfTaxonomicCategoryForJstree(Request request)
		...
		String candidateTaxonomicCategory = (String) request
				.getParam("queryIfTaxonomicCategoryForJstree");
		request.getLogger().error(
		...
		if (candidateTaxonomicCategory == null) {
			...
			try {
				...
				for (int j = 0; j < bindings.length(); j++) {
					...
					String speciesId = binding.getJSONObject("species")
							.getString("value");
	...
	/**
	 * This method will return if a class is a leaf or non-leaf. It is called by
	 * the method queryForNearbySpeciesCounts. I believe this was for testing
	 ...
	 * @throws JSONException
	 */
	public String queryIfTaxonomicCategory(Request request,
			String speciesInArray) throws JSONException {
		final Query query = config.getQueryFactory().newQuery(Type.SELECT);
		final NamedGraphComponent graph = query
				.getNamedGraph("http://was.tw.rpi.edu/ebird-taxonomy");
		final QueryResource subClassOf = query.getResource(RDFS_NS
		...
		try {
			...
			for (int j = 0; j < bindings.length(); j++) {
				JSONObject binding = bindings.getJSONObject(j);
				String speciesId = binding.getJSONObject("species").getString(
						"value");
				JSONObject mapping = new JSONObject();
				mapping.put("species", speciesId);
				data.put(mapping);
	 ...
	 */
	@QueryMethod
	public String querySpeciesByTaxonomicCategories(Request request)
			throws JSONException {
		final Query query = config.getQueryFactory().newQuery(Type.SELECT);
		final NamedGraphComponent graph = query
				.getNamedGraph("http://was.tw.rpi.edu/ebird-taxonomy");
		...
		if (request.getParam("species") != null
				&& ((JSONArray) request.getParam("species")).length() > 1) {
			JSONArray speciesParams = (JSONArray) request.getParam("species");
			final UnionComponent union = query.createUnion();
			// final UnionComponent union = query.createUnion();
			// final NamedGraphComponent graph2 =
			// query.getNamedGraph("http://was.tw.rpi.edu/ebird-taxonomy");
			// graph2.addGraphComponent(union);
			// GraphComponentCollection coll = union.getUnionComponent(0);
			GraphComponentCollection coll;
		...
		} else if (request.getParam("species") != null
				&& ((JSONArray) request.getParam("species")).length() == 1) {
			JSONArray speciesParams = (JSONArray) request.getParam("species");
		...
		try {
			...
			for (int j = 0; j < bindings.length(); j++) {
				...
				// binding.getJSONObject("label").getString("value");
	...
	/**
	 * Adds a species clause to the identified query
	 * @param request
	 * @param query
	 * @return
	 * @throws JSONException
	 */
	protected Query addSpeciesClause(Request request, Query query)
			throws JSONException {
		...
		if (speciesList != null && speciesList.length() > 1) {
			...
			for (int i = 0; i < speciesParams.length(); i++) {
				...
				String resultStr = queryIfTaxonomicCategory(request,
						speciesInArray);
				// if there are any bindings union up
				if (resultStr != "FAILURE") {
					request.getLogger().error(
					...
					if (data.length() > 0) {
						...
						coll.addPattern(species, subClassOf, addedSpecies);
						coll.addPattern(species, hasLabel, scientificName);
						// ebird data graph (already handled in the first part
						// of the query)
						// graph3.addPattern(addedSpecies, hasLabel,
						// scientificName);
	 ...
	 */
	@QueryMethod
	public String queryForNearbySpeciesCountsByLocality(Request request)
			throws JSONException {

		/*
		 * SELECT ?scientific_name ?count ?location ?date WHERE { graph
		 * <http://was.tw.rpi.edu/ebird-data-big> { ?measurement
		 * <http://rdfs.org/ns/void#inDataset>
		 * <http://was.tw.rpi.edu/source/bird
		 * -data/dataset/ebird-data/version/2012-Nov-4> . ?measurement
		 * <http://was
		 * .tw.rpi.edu/source/bird-data/dataset/ebird-data/version/2012
		 * -Nov-4/params/enhancement/1/countyCoded> "019" . ?measurement
		 * <http://
		 * was.tw.rpi.edu/source/bird-data/dataset/ebird-data/version/2012
		 * -Nov-4/params/enhancement/1/stateCoded> "MD" . ?measurement
		 * <http://was
		 * .tw.rpi.edu/source/bird-data/dataset/ebird-data/vocab/enhancement
		 * /1/locality> ?location . ?measurement
		 * <http://was.tw.rpi.edu/source/bird
		 * -data/dataset/ebird-data/vocab/enhancement/1/scientific_name>
		 * ?scientific_name . ?measurement
		 * <http://was.tw.rpi.edu/source/bird-data
		 * /dataset/ebird-data/vocab/enhancement/1/observation_count> ?count .
		 * ?measurement
		 * <http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data
		 * /vocab/enhancement/1/observation_date> ?date . ?measurement
		 * <http://www.w3.org/2003/01/geo/wgs84_pos#lat> ?lat . ?measurement
		 * <http://www.w3.org/2003/01/geo/wgs84_pos#long> ?long .
		 * 
		 * "Castle Haven Rd" } } ORDER BY ?location
		 */

		/*
		 * would it be better for matthew if i have a json with the locality and
		 * date as the key, and then a json array with key/values for location,
		 */
		...
		// URIs
		// final QueryResource dataSet =
		// query.getResource("http://sparql.tw.rpi.edu/source/akn/dataset/GBBC_CSV/version/2012-Oct-19");
		final QueryResource dataSet = query
				.getResource("http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/version/2012-Nov-4");
		// <http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/version/2012-Nov-4>
		final QueryResource inDataSet = query
				.getResource("http://rdfs.org/ns/void#inDataset");
		// http://rdfs.org/ns/void#inDataset
		final QueryResource countyCoded = query.getResource(e1_NS
				+ "countyCoded");
		final QueryResource stateAbbrev = query.getResource(e1_NS
		...
		final QueryResource wgsLat = query.getResource(WGS_NS + LAT);
		final QueryResource wgsLong = query.getResource(WGS_NS + LONG);
		final QueryResource birdCount = query.getResource(e2_NS
				+ "observation_count");
		final QueryResource obsDate = query.getResource(e2_NS
				+ "observation_date");

		// query based on the species name, you are doing this across two graphs
		final QueryResource hasCommonName = query.getResource(TXN_NS
				+ "CommonNameID");
		final QueryResource hasScientificName = query.getResource(e2_NS
				+ "scientific_name");
		final QueryResource hasLabel = query.getResource(RDFS_NS + "label");
	...
	/**
	 * This method returns all bird data measurements for a specific site, using
	 * uris in bbq state county, state, and uri (for site). It is done against
	 * the endpoint and not the loaded RDF model. This is called by the UI after
	 * a specific bird icon is selected.
	 * 
	 * @param request
	 * @return
	 * @throws JSONException
	...
	public String queryForSpeciesForASite(Request request) throws JSONException {
		...
		// final NamedGraphComponent graph =
		// query.getNamedGraph("http://was.tw.rpi.edu/ebird-data2");
		// currently we have only loaded bird data for California from eBird
		// (conversion to rdf was taking over a week)
		final NamedGraphComponent graph = query
				.getNamedGraph("http://was.tw.rpi.edu/ebird-data-ca");

		final Variable measurement = query
				.getVariable(QUERY_NS + "measurement");
		final QueryResource locality = query
				.getResource("http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/vocab/enhancement/1/locality");
		...
		request.getLogger().error(
				"in SpeciesDataProviderModule.queryForSpeciesForASite, query is : "
						+ query.toString());
		return config.getQueryExecutor(request).accept("application/json")
				.execute(query);
	}
	...
	/**
	 * This method queries and returns bird count data for the current county
	 * and state using "county" and "state" from bbq state. It checks that if
	 * the selected species is not a leaf, it adds a pattern for subclass of.
	 * When we add an ebird-data graph for each state, will need to add a graph
	 * for finding each, similar to EPA data where there is a graph for the
	 * graph uri and relations to states.
	 * 
	 * This is called only when the water or air data is initially selected, and
	 * bird data is sought after nearby the selected zip code. In the future we
	 * should enable the user to specify distance from selected site.
	 * 
	 * @param request
	 * @return
	 * @throws JSONException
	...
	public String queryForNearbySpeciesCounts(Request request)
		...
		query = addSpeciesClause(request, query);
		request.getLogger().error(
				"in SpeciesDataProviderModule.queryForNearbySpeciesCounts, query is : "
						+ query.toString());
		return config.getQueryExecutor(request).accept("application/json")
				.execute(query);
	}
	 ...
	 */
	@QueryMethod
	public String queryBirdTaxonomy(Request request) throws IOException,
			JSONException {
		...
		// in the visit which has query and request for characteristic module
		// (use archtype)
		...
		/*
		 * final Query query = config.getQueryFactory().newQuery(Type.SELECT);
		 * //Variables final Variable child = query.getVariable(VAR_NS+
		 * "child"); final Variable parent = query.getVariable(VAR_NS+
		 * "parent"); //URIs final QueryResource subClassOf =
		 * query.getResource(RDFS_NS + "subClassOf"); final QueryResource
		 * inDataSet = query.getResource("http://rdfs.org/ns/void#inDataset");
		 * //http://rdfs.org/ns/void#inDataset //build query Set<Variable> vars
		 * = new LinkedHashSet<Variable>(); vars.add(child); vars.add(parent);
		 * query.setVariables(vars); String parentClass = null; //query pattern
		 * //query.addPattern(site, inDataSet, dataSet); final
		 * NamedGraphComponent graph =
		 * query.getNamedGraph("http://was.tw.rpi.edu/bird-taxonomy");
		 * graph.addPattern(child, subClassOf, parent);
		 * 
		 * // ?m void:inDataSet
		 * <http://sparql.tw.rpi.edu/source/akn/dataset/GBBC_CSV
		 * /version/2012-Oct-19> String responseStr = FAILURE;
		 * //config.getQueryExecutor
		 * (request).accept("application/json").execute(query);
		 * 
		 * 
		 * String resultStr =
		 * config.getQueryExecutor(request).accept("application/json"
		 * ).execute(query); ////return
		 * config.getQueryExecutor(request).accept("application/json"
		 * ).execute(query);
		 * 
		 * 
		 * log.debug("Results: "+resultStr); if(resultStr == null) { return
		 * responseStr; } try { JSONObject results = new JSONObject(resultStr);
		 * JSONObject response = new JSONObject(); JSONArray data = new
		 * JSONArray(); response.put("success", true); response.put("data",
		 * data); results = results.getJSONObject("results"); JSONArray bindings
		 * = results.getJSONArray(BINDINGS); for(int
		 * i=0;i<bindings.length();i++) { JSONObject binding =
		 * bindings.getJSONObject(i); String childClass =
		 * binding.getJSONObject("child").getString("value"); try { parentClass
		 * = binding.getJSONObject("parent").getString("value"); }
		 * catch(Exception e) { } //if(labelStr == null) { // labelStr =
		 * sourceUri.substring(sourceUri.lastIndexOf('/')+1).replace('-', '.');
		 * //} JSONObject mapping = new JSONObject(); mapping.put("child",
		 * childClass); mapping.put("parent", parentClass); data.put(mapping); }
		 * responseStr = response.toString(); } catch (JSONException e) {
		 * log.error("Unable to parse JSON results", e); } return responseStr;
		 */
		// return
		// config.getQueryExecutor(request).accept("application/json").execute(query);

		/*
		 * String endpoint = ""; final Query query =
		 * config.getQueryFactory().newQuery(Type.SELECT);
		 * 
		 * String responseStr = FAILURE;
		 * config.getQueryExecutor(request).accept(
		 * "application/json").execute(query); String resultStr =
		 * config.getQueryExecutor(request).execute(endpoint, query);
		 * log.debug("Results: "+resultStr); if(resultStr == null) { return
		 * responseStr; } try { JSONObject results = new JSONObject(resultStr);
		 * JSONObject response = new JSONObject(); JSONArray data = new
		 * JSONArray(); response.put("success", true); response.put("data",
		 ...
		 * = results.getJSONArray(BINDINGS); for(int
		 * i=0;i<bindings.length();i++) { JSONObject binding =
		 * bindings.getJSONObject(i); String sourceUri =
		 * binding.getJSONObject(SOURCE_VAR).getString(VALUE); String labelStr =
		 * null; try { labelStr =
		 * binding.getJSONObject(LABEL_VAR).getString(VALUE); } catch(Exception
		 * e) { } if(labelStr == null) { labelStr =
		 * sourceUri.substring(sourceUri.lastIndexOf('/')+1).replace('-', '.');
		 * } JSONObject mapping = new JSONObject(); mapping.put("uri",
		 * sourceUri); mapping.put(LABEL_VAR, labelStr); data.put(mapping); }
		 * responseStr = response.toString(); } catch (JSONException e) {
		 * log.error("Unable to parse JSON results", e); } return responseStr;
		 */
	 ...
	 * 
	 * 
	 * //this works for eBird only final Query query =
	 * config.getQueryFactory().newQuery(Type.SELECT); //Variables final
	 * Variable site = query.getVariable(VAR_NS+SITE_VAR); String countyCode =
	 * (String) request.getParam("county"); String stateAbbr = (String)
	 * request.getParam("state");
	 * 
	 * assert(countyCode != null); assert(stateAbbr != null);
	 * 
	 * //URIs //final QueryResource dataSet = query.getResource(
	 * "http://sparql.tw.rpi.edu/source/akn/dataset/GBBC_CSV/version/2012-Oct-19"
	 * ); final QueryResource dataSet = query.getResource(
	 * "http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/version/2012-Nov-4"
	 * );
	 * //<http://was.tw.rpi.edu/source/bird-data/dataset/ebird-data/version/2012
	 * -Nov-4> final QueryResource inDataSet =
	 * query.getResource("http://rdfs.org/ns/void#inDataset");
	 * //http://rdfs.org/ns/void#inDataset final QueryResource countyCoded =
	 * query.getResource(e1_NS + "countyCoded"); final QueryResource stateAbbrev
	 * = query.getResource(e1_NS + "stateCoded"); final Variable measurement =
	 * query.getVariable(VAR_NS+"measurement"); final Variable count =
	 * query.getVariable(QUERY_NS+"count"); final Variable date =
	 * query.getVariable(QUERY_NS+"date"); final Variable commonName =
	 * query.getVariable(VAR_NS+"commonName"); final Variable scientificName =
	 * query.getVariable(VAR_NS + "scientific_name"); final Variable species =
	 * query.getVariable(VAR_NS + "species");
	 * 
	 * final Variable lat = query.getVariable(QUERY_NS+LAT); final Variable lng
	 * = query.getVariable(QUERY_NS+LONG); final Variable label =
	 * query.getVariable(QUERY_NS+LABEL_VAR);
	 * 
	 * final QueryResource wgsLat = query.getResource(WGS_NS+LAT); final
	 * QueryResource wgsLong = query.getResource(WGS_NS+LONG); final
	 * QueryResource birdCount = query.getResource(e2_NS+"observation_count");
	 * final QueryResource obsDate =
	 * query.getResource(e2_NS+"observation_date");
	 * 
	 * //query based on the species name, you are doing this across two graphs
	 * final QueryResource hasCommonName =
	 * query.getResource(TXN_NS+"CommonNameID"); final QueryResource
	 * hasScientificName = query.getResource(e2_NS+ "scientific_name"); final
	 * QueryResource hasLabel = query.getResource(RDFS_NS+ "label");
	 * 
	 * //build query Set<Variable> vars = new LinkedHashSet<Variable>();
	 * vars.add(measurement); vars.add(count); vars.add(date);
	 * vars.add(commonName); vars.add(scientificName); vars.add(lat);
	 * vars.add(lng); vars.add(species);
	 * 
	 * //vars.add(measurement);
	 * 
	 * query.setVariables(vars); //query pattern final NamedGraphComponent graph
	 * = query.getNamedGraph("http://was.tw.rpi.edu/ebird-data");
	 * graph.addPattern(measurement, inDataSet, dataSet);
	 * graph.addPattern(measurement, countyCoded, countyCode,null);
	 * graph.addPattern(measurement, stateAbbrev, stateAbbr,null);
	 ...
	 * graph.addPattern(measurement, hasCommonName, commonName);
	 * graph.addPattern(measurement, hasScientificName, scientificName);
	 * graph.addPattern(measurement, wgsLat, lat); graph.addPattern(measurement,
	 * wgsLong, lng);
	 */

	/**
	 * This method determines if the jstree selected species (updated in bbq
	 * "species") has any sibling species with data. If so, it returns the list
	 * of siblings for user confirmation for plotting. This method is only
	 * called when there is one selected species and there is no data returned
	 * for it.
	 * 
	 * @param request
	...
	public String queryIfSiblingsExist(Request request) throws JSONException {
		...
		// *****only call this if there is only one! so here we assume there is
		// one parameter
		...
		final QueryResource hasCommonName = query.getResource(TXN_NS
				+ "CommonNameID");
		final Variable siblingScientificName = query.getVariable(VAR_NS
				+ "scientific_name");
		final QueryResource countyCoded = query.getResource(e1_NS
				+ "countyCoded");
		final QueryResource stateAbbrev = query.getResource(e1_NS
		...
		// query.createVariableExpression("count(?measurement) as ?"+
		// measurement);
		final QueryResource subClassOf = query.getResource(RDFS_NS
				+ "subClassOf");
		...
		graph2.addPattern(measurement, hasScientificName, scientificName); // selected
																			// species
		graph2.addPattern(measurement, hasScientificName, siblingScientificName); // sibling
																					// only
																					// if
																					// type
																					// matches
		...
		try {
			...
			for (int i = 0; i < bindings.length(); i++) {
				...
				String siblingVar = binding.getJSONObject("sibling").getString(
						"value");
				String commonNameVar = binding.getJSONObject("commonName")
						.getString("value");
	 ...
	 * if(speciesParams != null && speciesParams.length() > 0) { //throw new
	 * IllegalArgumentException("The source parameter must be supplied"); //note
	 * that this is going to be a json object JSONObject jsonSpecies = new
	 * JSONObject(species); //iterate over the species and create a union clause
	 * //iterate over jason entity uris
	 * 
	 * 
	 * 
	 * 
	 * after you select bbq arguments scientificName subClassOf <selection>
	 * graph2.addPattern(species, hasLabel, scientificName);
	 * .createVariableExpression cf. the Regulation module
	 * 
	 * }
	 */

	/**
	 * This method returns the immediate subclasses of the top-level class, in
	 * the current case http://ebird#birdTaxonomy.
	 * 
	 * @param request
	 * @return json array with json object where key/value success/true and data
	 *         key with array of objects with id, label, and parent triples as
	 *         key/value pairs.
	 * 
	 * @throws IOException
	 * @throws JSONException
	 */
	@QueryMethod
	public String queryeBirdTaxonomyRoots(Request request) throws IOException,
			JSONException {
		final Query query = config.getQueryFactory().newQuery(Type.SELECT);
		// Variables
		...
		request.getLogger().info(
				"Query object in queryeBirdTaxonomyRoots is: "
						+ query.toString());
	...
	/**
	 * This method returns the subclass data for the uri added to bbq state
	 * "queryeBirdTaxonomySubClasses", and is necessary for the dynamic jstree
	 * selection.
	 * 
	 * @param request
	 ...
	 */
	@QueryMethod
	public String queryeBirdTaxonomySubClasses(Request request)
			throws IOException, JSONException {

		String classRequiresSubclassesString = (String) request
				.getParam("queryeBirdTaxonomySubClasses");
		...
		final QueryResource subClassOf = query.getResource(RDFS_NS
				+ "subClassOf");
		final QueryResource hasCommonName = query
				.getResource("http://lod.taxonconcept.org/ontology/txn.owl#CommonNameID");
	 ...
	 */
	@QueryMethod
	public String queryeBirdTaxonomy(Request request) throws IOException,
			JSONException {
		final Query query = config.getQueryFactory().newQuery(Type.SELECT);
		// Variables
		final Variable id = query.getVariable(VAR_NS + "child");
	...
	public List<Domain> getDomains(final Request request) {
		...
		Domain bird = config.getDomain(URI.create(BIRD_NS), true);
		bird.setLabel("Bird");
		...
		Domain fish = config.getDomain(URI.create(FISH_NS), true);
		fish.setLabel("Fish");
	...
	/**
	 * This method adds a bird icon as a UI resource, and adds data type "bird"
	 * and icon label "Bird Species" for UI.
	 * 
	 * @param domain
	 */
	protected void addDataTypesBirds(final Domain domain) {
		...
		// bird is the identifier of the checkbox, and Bird Species is the label
		// for the icon type
	}

	/**
	 * This method adds a fish icon as a UI resource, and adds datatype "fish"
	 * and icon label "Fish Species" for UI.
	 * 
	 * @param domain
	 */
	protected void addDataTypesFish(final Domain domain) {
		...
		// the first string is used in speciesHierarchy.js where icon variable
		// is set.
	}
	...
	/**
	 * Provides the hierarchical behavior for the species module's facet.
	 * @param request
	 * @param action
	 * @return
	 */
	@HierarchicalMethod(parameter = "species")
	public Collection<HierarchyEntry> queryeBirdTaxonomyHM(
		...
		if (action == HierarchyVerb.ROOTS) {
			...
			// items.add(entry);
			// }
			return queryeBirdTaxonomyHMChildren(request,
	...
	protected Collection<HierarchyEntry> queryeBirdTaxonomyHMChildren(
	...
	protected Collection<HierarchyEntry> searcheBird(final Request request,
			final String str) {
		...
		union.getUnionComponent(0).addFilter(
				"bif:contains(?label,\"'" + str + "*'\")");
		union.getUnionComponent(1).addPattern(uri, hasCommonName, label);
		union.getUnionComponent(1).addFilter(
				"bif:contains(?label,\"'" + str + "*'\")");
		query.addOrderBy(label, SortType.ASC);
	...
	protected Collection<HierarchyEntry> eBirdPathToNode(final Request request,
			final String node) {
		final Collection<HierarchyEntry> entries = new ArrayList<HierarchyEntry>();
		final Query query = config.getQueryFactory().newQuery(Type.SELECT);
		final Variable uri = query.createVariable(VAR_NS + "uri");
		...
		final QueryResource rdfsSubClassOf = query.getResource(RDFS_NS
				+ "subClassOf");
		final QueryResource skosPrefLabel = query.getResource(SKOS_NS
				+ "prefLabel");
