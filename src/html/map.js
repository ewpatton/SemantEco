...
function onchange_species_selection() {
	...
	if (species == "Canadian-goose") {
		...
		$.getJSON("canada_goose_wa_huc8.json", function(ret) {
			var huc = ret.HUC_8;
			$.getJSON("water-bodies.json", function(json) {
					...
					$(huc).each(function() {
						if (feature.properties.ReachCode.indexOf(this) === 0) {
							...
							if (lng < northEast.lng() && lat < northEast.lat()
								...
								highlightPolygon(coords, feature.properties, highlight);
						}
					});
				});
				$("#spinner").css("display","none");
 ...
 * @param polygon the polygon to highlight
 * polygon ::= [outline, hole-1, hole-2, ..., hole-n]
 * outline, hole-k ::= [coord-1, coord-2, ..., coord-m]
 * coord-k ::= [longitude, latitude]
 * longitude, latitude ::= floating number such as -119.08
 * @param properties the properties of the water body recorded in the GeoJSON object
 * @param record an array used to store all the polygons currently highlighted, so that we can 
 * un-highlight them later.
 */
function highlightPolygon(polygon, properties, record) {
	...
	$(polygon).each(function () {
		...
		var polygon1 = null;
		if (first) {
			polygon1 = new GPolygon(poly_coords, "#fff", 0, 0, "#00f", 0.5);
			...
			GEvent.addListener(polygon1, "click", function() {
				map.openInfoWindowHtml(polygon1.getBounds().getCenter(), makeTable(properties));
			});
			first = false;
		} else {
			polygon1 = new GPolygon(poly_coords, "#fff", 0, 0, "#fff", 0.5);
		}
 ...
 * @param properties the properties where the data in the table come from
