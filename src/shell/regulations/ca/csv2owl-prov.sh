...
# Usage:
#
#   csv2owl4reg-prov.sh epa-regulation.csv
#   (produces epa-regulation.owl.pml.ttl)
#

usage_message="usage: `basename $0` the name of the csv file"
...
if [ ! -e $file.pml.ttl ]; then
   ...
   if [ `man stat | grep 't timefmt' | wc -l` -gt 0 ]; then
      ...
      echo "   pmlj:hasInferenceEngine conv:csv2owl4reg_$myMD5;"                                >> $file.pml.ttl
      #TO DO: what is a proper InferenceRule 
      #echo "   pmlj:hasInferenceRule   " 							>> $file.pml.ttl
      ...
      echo "."                                                                                  >> $file.pml.ttl
      echo                                                                                      >> $file.pml.ttl
      echo "conv:csv2owl4reg_$myMD5"                                                           >> $file.pml.ttl
      echo "   a pmlp:InferenceEngine, conv:csv2owl4reg;"                                          >> $file.pml.ttl
      echo "   dcterms:identifier \"md5_$myMD5\";"                                              >> $file.pml.ttl
      echo "."                                                                                  >> $file.pml.ttl
      echo                                                                                      >> $file.pml.ttl
      echo "conv:csv2owl4reg rdfs:subClassOf pmlp:InferenceEngine ."                               >> $file.pml.ttl
      echo                                                                                      >> $file.pml.ttl
else 
