...
# Usage:
#
#   table2csv-prov.sh epa-regulation.html
#   (produces epa-regulation.csv.pml.ttl)
#

usage_message="usage: `basename $0` Name of the html file Name of the convertor"
...
if [ ! -e $file.pml.ttl ]; then
   ...
   if [ `man stat | grep 't timefmt' | wc -l` -gt 0 ]; then
     ...
     wasControlled="wasControlledBy$requestID"
      ...
      echo "   pmlj:hasInferenceEngine conv:$convEngine_$myMD5;"                                >> $file.pml.ttl
      ...
      echo "."                                                                                  >> $file.pml.ttl
      echo                                                                                      >> $file.pml.ttl
      echo "conv:$convEngine_$myMD5"                                                           >> $file.pml.ttl
      echo "   a pmlp:InferenceEngine, conv:$convEngine;"                                          >> $file.pml.ttl
      echo "   dcterms:identifier \"md5_$myMD5\";"                                              >> $file.pml.ttl
      echo "."                                                                                  >> $file.pml.ttl
      echo                                                                                      >> $file.pml.ttl
      echo "conv:$convEngine rdfs:subClassOf pmlp:InferenceEngine ."                               >> $file.pml.ttl
      echo                                                                                      >> $file.pml.ttl
else 
