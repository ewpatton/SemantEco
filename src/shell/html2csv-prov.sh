...
usage_message="usage: `basename $0` fileName"
...
if [ ! -e $file.pml.ttl ]; then
   ...
   if [ `man stat | grep 't timefmt' | wc -l` -gt 0 ]; then
     ...
     wasControlled="wasControlledBy$requestID"
      ...
      echo "   pmlj:hasInferenceEngine html2csv_$myMD5;"                                        >> $file.pml.ttl
      echo "   pmlj:hasInferenceRule   Extract_and_Reformat" 									>> $file.pml.ttl
