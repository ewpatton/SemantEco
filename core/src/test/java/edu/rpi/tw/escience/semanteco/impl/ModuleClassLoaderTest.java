...
public class ModuleClassLoaderTest extends TestCase {
	...
	protected File createTestJar() throws Exception {
		...
		zos.closeEntry();
		zos.putNextEntry(new ZipEntry("edu/rpi/tw/escience/semanteco/impl/ModuleManagerImplTest.class"));
		cls = getClass().getClassLoader().getResourceAsStream("edu/rpi/tw/escience/semanteco/impl/ModuleManagerImplTest.class");
		while((read = cls.read(buffer)) > 0) {
			zos.write(buffer, 0, read);
		}
		cls.close();
